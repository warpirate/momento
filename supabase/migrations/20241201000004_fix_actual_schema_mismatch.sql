-- Fix the actual schema mismatch based on current database structure
-- This migration fixes the sync functions to match the actual database schema

-- Drop and recreate pull_changes function with correct types for actual schema
...
begin
  ...
  -- Pull signals with correct types for actual database schema
...
-- Drop and recreate push_changes function with correct types for actual schema
...
begin
  ...
  -- Process signals with correct type handling for actual database schema
  if (changes->'entry_signals'->>'created') is not null then
    for _signal in select * from jsonb_array_elements(changes->'entry_signals'->'created') loop
      ...
      values (
        ...
        case when (_signal->'tags') is null or jsonb_typeof(_signal->'tags') = 'null' then array[]::text[]
